...
using MemcacheAdmin.Filters;
using MemcacheAdmin.Models;
using MemcacheAdmin.Security;
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
        {
            if (ModelState.IsValid && WebSecurity.Login(model.UserName, model.Password, persistCookie: model.RememberMe))
            {
                ...
                //var r = HttpContext.User = MemcacheUser.Load(model);
        ...
        public ActionResult ManageServers()
        {
            ViewBag.ReturnUrl = Url.Action("ManageServers");
            if (HttpContext.User.Identity.IsAuthenticated)
            {
                InitServers();
                ServersModel model = new ServersModel();
        ...
        [HttpPost]
        public JsonResult AjaxLookup(string serverId)
        {
            if (HttpContext.User.Identity.IsAuthenticated)
            {
                InitServers();
                ...
                if (current != null)
                {
                    JsonResult result = new JsonResult()
                    {
                        ...
                        {
                            id = current,
                            value = string.Format("{0} - {1}:{2}", current.Name, current.IPAddress, current.Port),
                            code = true
                        }
                    };
        ...
        [HttpDelete]
        public JsonResult AjaxDelete(string serverId)
        {
            if (HttpContext.User.Identity.IsAuthenticated)
            {
                InitServers();
